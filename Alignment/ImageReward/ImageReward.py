...
class ImageReward(nn.Module):
    ...
    def score(self, prompt, image):
        ...
        image_embeds = self.blip.visual_encoder(image.half())
        ...
        txt_features = text_output.last_hidden_state[:,0,:].half() # (feature_dim)
        rewards = self.mlp(txt_features)
        rewards = (rewards - self.mean) / self.std
    ...
    def inference_rank(self, prompt, generations_list):
        ...
        for generation in generations_list:
            ...
            if isinstance(generation, Image.Image):
                ...
                raise TypeError(r'This image parameter type has not been supportted yet. Please pass PIL.Image or file path str.')
            image = self.preprocess(pil_image).unsqueeze(0).to(self.device)
            image = image.to(dtype=self.blip.visual_encoder.dtype)
            image_embeds = self.blip.visual_encoder(image)
